...
7. **Start the apps** (backend + storefront, in parallel):
   ...
   pnpm --filter capri-storefront dev  # Vite storefront on :5198
   ```
...
### `apps/capri-storefront` (React + Vite)

Fully static and self-contained: the catalog ships as JSON under `public/`
(regenerated by the daily `catalog-sync` GitHub Action), so **no environment
variable is required to build or run it**. All `VITE_*` vars below are optional
overrides that fall back to working defaults.

| Resource | Purpose | Required for |
|---|---|---|
| `VITE_CATALOG_URL` | Serve the catalog JSON from a remote origin instead of `public/` | Multi-tenant / SaaS deploys |
| `VITE_ADMIN_USER` + `VITE_ADMIN_PASS_SHA256` | Admin panel credentials (`/admin`) | **Any public deploy** — the defaults are baked into the bundle |
| `VITE_ADMIN_AUTH_URL` | Point the admin login at a real auth endpoint | Replacing the client-side gate |
| `VITE_ADMIN_2FA` + `VITE_ADMIN_2FA_URL` | Enable a second factor on admin login | Optional hardening |

The admin gate is client-side only — it is not a real security boundary. See
the note at the top of `src/admin/adminAuth.js`.
