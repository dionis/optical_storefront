...
// Cristal: la silueta interior como superficie translúcida.
function lensMesh(dim, side, rimW) {
  const { eye, lensH, shape } = dim;
  const geo = new THREE.ShapeGeometry(
    shapeFromPoints(lensPoints(eye - 2 * rimW, lensH - 2 * rimW, shape, side)), 12
  );
...
export function buildFrame(product, hex) {
  ...
  for (const side of [-1, 1]) {
    const rim = rimMesh(dim, side, rimW, depth, material);
